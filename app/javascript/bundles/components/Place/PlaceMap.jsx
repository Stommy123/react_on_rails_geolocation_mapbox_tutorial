...
import { mapboxToken, mapStyle, geolocationOptions, loadPosition } from '../../utilities/utils';
...
class PlaceMap extends Component {
    ...
    map.on('load', _ => {
      ...
      initialPlaces.forEach(place => {
        ...
        if (!map.getLayer(placeType)) {
          ...
          map.on('mouseenter', placeType, _ => (map.getCanvas().style.cursor = 'pointer'));
          map.on('mouseleave', placeType, _ => (map.getCanvas().style.cursor = String()));
        }
      });
  ...
  fetchPlaces = async _ => {
    ...
    const {
      data,
      data: { features }
    } = await axios.get(`/places.json?lat=${lat}&lng=${lng}`);
  ...
  filterPlaces = placeType => e => {
    ...
    this.map.setLayoutProperty(placeType, 'visibility', visiblePlaces.includes(placeType) ? 'none' : 'visible');
    ...
    return (
      <div id="placeMap-container">
        ...
        {placeTypes.map(place => (
          <PlaceFilter key={place} place={place} visiblePlaces={visiblePlaces} filterPlaces={this.filterPlaces} />
        ))}
      </div>
    );
  }
