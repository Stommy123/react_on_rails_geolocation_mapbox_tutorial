...
class Tracker extends Component {
  ...
  handleLeavePage = _ => {
    clearInterval(this.interval);
    axios.post('/locations', { location: { latitude: null, longitude: null } }, { headers });
  };
  ...
  trackLocation = _ => {
    ...
    const success = async pos => {
      ...
      const { data } = await axios.post('/locations', { location: { latitude, longitude } }, { headers });
      this.setState({
        latitude: data.latitude,
        longitude: data.longitude
      });
      flyTo({ coordinates: [data.longitude, data.latitude] });
    };
